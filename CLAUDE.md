...
Single-page React 18 + Vite + TypeScript app, organised as a shell plus modules (the big-file refactor in `docs/REFACTOR.md` is complete through Phase 7; feature flags remain). All source lives under `src/`; the repo root holds only configs, `index.html`, `docs/`, `scripts/`, `tests/` and `public/`.
...
- `src/state/*.tsx`: seven focused React contexts (Toast, Settings, AuthSync, Library, Progress, Selection, Playback) plus two publish slots (`FretboardContext`, `ReadoutContext`). No god object. Nesting order lives in `App.tsx`'s provider tree and `docs/REFACTOR-BLUEPRINT.md`. Migrate contexts outermost-first when touching types: a `createContext(null)` whose hook is consumed in a `.tsx` file infers as `never` until every provider above it is typed.
...
- `src/lib/*.ts`: pure helpers (`routing`, `analytics`, `share`, `utils`, `store`, `supabase`); `src/data/*`: data constants (`faq` stays `.js` because the node prerender script imports it, `groups`, ...).
...
TypeScript is strict for the shared core (`theory.ts`, `lib/`, contexts) and relaxed (`strict: false`) for the leaf layer (views, `App.tsx`, `fretboard.tsx`), so the domain model documents itself while the UI leaves migrated without churn. `tsconfig` uses `allowJs` with `checkJs: false`, and `allowImportingTsExtensions` (Vite resolves `.ts`/`.tsx` at import), so a `.js` -> `.ts` rename means updating any importer that spelled the old extension.
